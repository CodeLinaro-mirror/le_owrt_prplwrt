...
enable_ap() {
  R "ba-cli -j -l Device.WiFi.AccessPoint.${1}.Enable=1 | grep -q Enable && echo 'Device.WiFi.AccessPoint.${1} enabled'"
}
...
disable_ap() {
  R "ba-cli -j -l Device.WiFi.AccessPoint.${1}.Enable=0 | grep -q Enable && echo 'Device.WiFi.AccessPoint.${1} disabled'"
}
...
check_ap_ref_ssid() {
  R "
    i=10
    while [ \$i -gt 1 ]; do
      ba-cli -j -l Device.WiFi.AccessPoint.${1}.SSIDReference+.Status? |
...
get_ssid_ref() {
  msg=$(R "ba-cli -j -l Device.WiFi.AccessPoint.${1}.SSIDReference+.Status?")
  echo "$msg" | sed '/^$/d'
}

# Print SSIDs status
get_ssid_status() {
  R "ba-cli -j -l Device.WiFi.SSID.?0 | jsonfilter -e @[0]'[@.Alias != \"ep2g0\" && @.Alias != \"ep5g0\" && @.Alias != \"ep6g0\"].Status'" | LC_ALL=C sort
}
