...
Check if hostap pid has changed or not before custom arguments is set:

  $ if [ "$(R pgrep -f 'hostapd')" = "$hostap_pid" ]; then echo "true"; else echo "hostapd restarted during the test !"; fi
  true
...
Saving hostap pid:
  ...
  $ hostap_pid=$(R pgrep -f 'hostapd')
  $ R logger -t cram "hostap PID after custom arguments test: $hostap_pid"
