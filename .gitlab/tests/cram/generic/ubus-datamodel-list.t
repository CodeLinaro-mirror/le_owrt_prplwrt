...
Check that ubus has expected datamodels available:
  ...
  $ R "ba-cli 'dump -r .' | cut -b 34- | grep '[[:upper:]]' | grep -v -e '\.[[:digit:]]' -e '^Cellular' -e 'Device.Cellular' -e 'WiFi.Vendor.Daemon' -e 'WiFi.Vendor.ModuleMode' -e '^Bridging.Bridge.[SV]' -e 'WiFi.Vendor.ReconfManager'"
  ACLManager.
  ACLManager.Role.
  Bridging.
  Bridging.Bridge.
