...
Verify TCP conntrack entry is present:
  ...
  $ R "i=0; n=0; while [ \$i -lt 10 ]; do n=\$(conntrack -L -p tcp --dport 8000 2>/dev/null | wc -l); [ \$n -ge 1 ] && break; i=\$((i+1)); sleep 1; done; [ \$n -ge 1 ] && echo entry-present || echo entry-count=\$n"
  entry-present
...
Configure port trigger again on mapped port 8001 to verify conntrack flush on disable:
  ...
  > ba-cli 'NAT.PortTrigger.ct-pt-test2.Rule.ct-pt-rule2.Port=8001'
  ...
  $ R "iptables -L FORWARD_PortTrigger -n | grep 8001 | wc -l"
  2

Establish TCP connection on port 8001 to create conntrack entry:

  $ nc -l -p 8001 -s $TARGET_LAN_TEST_HOST > /dev/null 2>&1 &
  $ sleep 1
  $ timeout 15 nc -s $TESTBED_WAN_IP $TARGET_WAN_IP 8001 > /dev/null 2>&1 &
  $ sleep 2

Verify TCP conntrack entry is present:

  $ R "i=0; n=0; while [ \$i -lt 10 ]; do n=\$(conntrack -L -p tcp --dport 8001 2>/dev/null | wc -l); [ \$n -ge 1 ] && break; i=\$((i+1)); sleep 1; done; [ \$n -ge 1 ] && echo entry-present || echo entry-count=\$n"
  entry-present

Kill nc and disable port trigger:

  $ pkill -f 'nc.*8001' 2>/dev/null; sleep 1
  $ script --command "ssh -t root@$TARGET_LAN_IP 'ba-cli \"NAT.PortTrigger.ct-pt-test2.Enable=0\"'" > /dev/null; sleep 2

Check that FORWARD_PortTrigger rules for port 8001 are gone:
