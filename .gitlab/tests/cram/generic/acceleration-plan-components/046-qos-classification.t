...
Create QoS.Classification and Routing.Policy to redirect packets with destination 8.8.8.8 to the table Device.Routing.Router.1
  $ R "ubus-cli 'QoS.Classification.+{Alias = \"ipv4_forw_policy\", DestIP = 8.8.8.8, Enable = 1, IPVersion = 4, ForwardingPolicy = 250}'" > /dev/null 2>&1
  $ R "ba-cli 'Routing.Policy.+{Alias = \"ipv4_forw_policy\", Priority = 500, ForwardingPolicy = 250, RouterRef = \"Device.Routing.Router.1\", Enable = 1}'" > /dev/null 2>&1
...
Check that a rule is created in the mangle table to mark traffic to 8.8.8.8 with skb mark 0x3f00/0x3fc0
  ...
  MARK       0    --  0.0.0.0/0            8.8.8.8              MARK xset 0x3e80/0x3fc0

Check that a routing policy is created for traffic marked with 0x3e80/0x3fc0
  $ R "ip rule | grep 0x3fc0"
  500:	from all fwmark 0xfa/0x3fc0 lookup main
