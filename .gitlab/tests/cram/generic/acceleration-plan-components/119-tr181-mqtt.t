...
Sleep for a moment to ensure subscriber is ready before publishing messages:
...
Start a ubus subscriber in background to listen for MQTT messages received by the MQTT client and check for the published message:
...
Publish message using MQTT Publish RPC and check syslog for publish callback message:

  $ R "ba-cli 'MQTT.Client.local_broker.Publish(Topic=\"SAH_CRAM_tests\", Payload=\"testMessage\", QoS=0, Retain=false)'" > /dev/null

Wait for the subscription to take effect and the message to be received by the subscriber, then check the output:
...
Teardown, Delete MQTT client and subscription:

  $ R "ba-cli 'MQTT.Client.*.-'" > /dev/null

Teardown, Kill mosquitto_sub using PID file:
