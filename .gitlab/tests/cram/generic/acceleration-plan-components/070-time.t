...
Check that we've expected datamodel:
  ...
  $ R "ba-cli 'dump -r Time.' | cut -b 34- | sed 's/\.$//' | grep -v '^$' | sort"
  Time
  Time.Client
  Time.Client.1
  ...
  $ R "ba-cli -l 'Time.Client.1.Port?;Time.Client.1.Status?;Time.Client.1.Servers?;Time.Client.1.Alias?;Time.Client.1.Mode?' | grep -v '^$' | sort"
  ...
  $ R "ba-cli -l 'Time.Server.*.Port?;Time.Server.*.Status?;Time.Server.*.Alias?;Time.Server.*.Mode?' | grep -v '^$' | sort"
  123
  123
  123
  Unicast
...
Check that we've correct Time.CurrentLocalTime:
  ...
  $ time=$(R "ba-cli -l 'Time.CurrentLocalTime?' | grep -v '^$'")
