...
Test write-once read-only functionality with DHCPv6.Client Alias parameter in usp:
...
Attempt to modify the Alias (should fail - write-once read-only):
  ...
  $ R "usp-cli 'Device.DHCPv6.Client.WriteOnceTestV6.Alias=ModifiedValueV6' 2>&1 | grep -Ev '^>|^$|^\+'"
...
Cleanup - remove only the instance created in this test:
  ...
  $ R "usp-cli 'Device.DHCPv6.Client.WriteOnceTestV6.-' | grep -Ev '^>|^$'"
  Device.DHCPv6.Client.*. (re)
