...
ssh root@$TARGET_LAN_IP 'exec <&- >&- 2>&- ; (while [ "$(ubus call PPP.Interface.1 _get | jsonfilter -e @[*].ConnectionStatus)" !=  "Connected" ] ; do sleep 10; done ; sleep 10; cp /var/etc/dsmasq.conf.* /etc/ ; /bin/getDebugInformation -n -w -p -m -l -o - > /etc/debug_triggered.txt; /tmp/luadumper.lua  >>/etc/debug_triggered.txt;  ) &'
